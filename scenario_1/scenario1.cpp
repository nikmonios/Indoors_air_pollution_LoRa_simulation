...
main (int argc, char *argv[])
{
  ...
  ******************/
  ...
  allocator->Add (Vector (50,50,12)); // x_gw = 50, y_gw = 50, z_gw = 12
  ...
  for (NodeContainer::Iterator j = endDevices.Begin (); j != endDevices.End (); ++j)
  {
	  ...
	  if(iter_x == 9) //when we have done 10 rooms on x axis, change row
	  {
		  iter_y++; // changer raw
		  iter_x = 0; //reset counter
	  }

	  if(iter_y == 9) //when we have done all 10 rows, we need to change floor
	  {
